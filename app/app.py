...
try:
    ...
    st.error("⚠️ OpenCV not found. Installing opencv-python-headless...")
    try:
        import subprocess
        import sys
        ...
        st.error(f"❌ Failed to install OpenCV: {install_error}")
        st.error("Please check deployment requirements: opencv-python-headless==4.8.1.78")
        st.stop()
